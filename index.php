...
// 안전하게 ORDER BY 결정 — DB에 컬럼이 있는지 확인 후 선택
  ...
  $best_products = db_select("SELECT * FROM contents ORDER BY {$best_order} LIMIT 3");
  ...
  <main class="main_wrapper">
    ...
    <div class="recommend_main_wrapper" style="position: relative; max-width: 1200px; margin: 0 auto; padding: 20px;">
      ...
      <div class="product-slide-container" style="position: relative; overflow: hidden; padding: 0 50px;">
        <button class="slide-btn slide-prev" style="position: absolute; left: 0; top: 50%; transform: translateY(-50%); z-index: 100; background: white; border: 2px solid #ff6b6b; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.15); transition: all 0.3s;" aria-label="이전">
          <i class="fas fa-chevron-left" style="color: #ff6b6b; font-size: 16px;"></i>
        </button>
        <button class="slide-btn slide-next" style="position: absolute; right: 0; top: 50%; transform: translateY(-50%); z-index: 100; background: white; border: 2px solid #ff6b6b; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.15); transition: all 0.3s;" aria-label="다음">
          <i class="fas fa-chevron-right" style="color: #ff6b6b; font-size: 16px;"></i>
        </button>
      ...
      <div class="slide-indicators" style="display: flex; justify-content: center; gap: 8px; margin-top: 20px;">
        ...
        <button class="slide-indicator <?= $i === 0 ? 'active' : '' ?>" data-page="<?= $i ?>" style="width: 10px; height: 10px; border-radius: 50%; border: none; background: <?= $i === 0 ? '#ff6b6b' : '#ddd' ?>; cursor: pointer; transition: all 0.3s;"></button>
        <?php endfor; ?>
      </div>
      ...
      (function() {
        ...
        function updateSlide() {
          ...
          indicators.forEach((ind, idx) => {
            ...
            ind.style.background = idx === currentPage ? '#ff6b6b' : '#ddd';
    ...
    <section class="notice-section" style="max-width: 1200px; margin: 0 auto 60px; padding: 20px;">
      ...
      <ul class="notice-list" style="list-style: none; padding: 0; margin: 0;">
        ...
        <li style="border-bottom: 1px solid #eee; padding: 12px 0; display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 1rem; color: #333; flex: 1;">
                <?= htmlspecialchars($notice['title']) ?>
            </span>
            <span style="font-size: 0.9rem; color: #888;"><?= substr($notice['reg_date'], 0, 10) ?></span>
        </li>
        <?php endforeach; ?>
      </ul>
    </section>
